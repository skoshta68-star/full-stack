        ...
        <div className="rounded-xl bg-white px-8 py-10"
          ...
          <div className="text-center mb-8">
            <div className="w-12 h-12 mx-auto mb-4 rounded-xl flex items-center justify-center"
              ...
              <Icons.HelpCircle className="w-6 h-6 text-white" strokeWidth={1.5} />
            </div>
            <h1 className="text-xl font-display font-bold" style={{ color: '#1e293b' }}>Forgot password?</h1>
            <p className="mt-1 text-sm" style={{ color: '#64748b' }}>Enter your email and we'll send you a reset link</p>
          </div>
          ...
          {sent ? (
            <div className="text-center">
              ...
              <p className="text-sm font-medium" style={{ color: '#374151' }}>Check your email</p>
              <p className="text-xs mt-1" style={{ color: '#6b7280' }}>If that email is registered, you'll receive a reset link shortly.</p>
              <Link to="/login" className="inline-block mt-4 text-sm font-semibold hover:underline" style={{ color: '#6366f1' }}>Back to login</Link>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input type="email" placeholder="you@example.com" value={email}
                  ...
                  style={{ borderColor: errors.email ? '#ef4444' : '#d1d5db', background: '#ffffff', color: '#111827' }}
                  onFocus={(e) => { if (!errors.email) { e.target.style.borderColor = '#6366f1'; e.target.style.boxShadow = '0 0 0 3px rgba(99,102,241,0.1)'; } }}
              ...
              <motion.button type="submit" disabled={loading}
                ...
                style={{ background: 'linear-gradient(135deg, #6366f1 0%, #4f46e5 100%)', boxShadow: '0 4px 12px rgba(99, 102, 241, 0.3)' }}>
                {loading ? <><Icons.Loader2 className="w-4 h-4 animate-spin" /><span>Sending...</span></>
                  : <><Icons.ArrowRight className="w-4 h-4" /><span>Send reset link</span></>}
              </motion.button>
            </form>
          )}
